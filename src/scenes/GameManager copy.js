...
export default class GameManager extends Phaser.Scene{
    ...
    // FIXME: When changing scenes, we don't need to get the information from the scenes themselves
    // Perhaps this.game.map should actually hold objects like
    /*  {
            scene: the scene in that space (null by default)
            coordinate: a Vector2 of the xy coordinate (may be redundant since we need map[x][y] to get access this value anyway)
            icon: the sprite icon on the map (this will be important for retrieving what directions are unlocked)
        }
    */
    goUp(scene){
        console.log(scene);
